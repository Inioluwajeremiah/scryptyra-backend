...
exports.login = async (req, res, next) => {
  try {
    ...
    sendEmail(user.email, 'loginAlert', user.name, getIP(req), req.headers['user-agent']).catch(() => {});
...
exports.resendVerification = async (req, res, next) => {
  try {
    ...
    if (user.isEmailVerified) {
      return res.status(200).json({ status: 'success', message: 'Your email is already verified.' });
    }
    ...
    if (
      ...
      return next(new AppError('A verification email was sent very recently. Please wait a moment before trying again.', 429));
    }
    ...
    res.status(200).json({ status: 'success', message: 'Verification email sent. Please check your inbox.' });
  } catch (err) {
    next(err);
  }
};
